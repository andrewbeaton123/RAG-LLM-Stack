...
def test_set_retriever():

    llm = LMStudioLLM(base_url= "http://mocked_url:1234/v1")
    
    llm.set_retriever("test", 5)
    

    assert llm._retriever == "test"
    assert llm._retrieval_k == 5



def test__unpack_doc_text_text_input():

    llm = LMStudioLLM(base_url= "http://mocked_url:1234/v1")
    
    assert llm._unpack_doc_text("Test string") == "Test string"


def test__unpack_doc_text_None_input():

    llm = LMStudioLLM(base_url= "http://mocked_url:1234/v1")
    
    assert llm._unpack_doc_text(None) == ""


def test__unpack_doc_text_dict_input():

    llm = LMStudioLLM(base_url= "http://mocked_url:1234/v1")
    
    assert llm._unpack_doc_text({"text": "Test string"}) == "Test string"
    assert llm._unpack_doc_text({"page_content": "Test string"}) == "Test string"
    assert llm._unpack_doc_text({"content": "Test string"}) == "Test string"
    assert llm._unpack_doc_text({"Test string"}) == "{'Test string'}"
